...
def evaluateResults(params):
    k_r, k_w, pat, max_it, (idx, g) = params

    print(f"I'm {current_process()} and I'm checking graph {idx}")

    init_cross, best_cross, best_cross_it = getLayout(g, k_r, k_w, max_it, pat)
        
    data = [idx, k_r, k_w, pat, max_it, init_cross, best_cross, best_cross_it]
...
def main(experiment_num, output_filepath):
    ...
    gs = [(idx, ds.getGraph(idx)) for idx in idxs]

    #col_names = ["graph_id", "k_relax", "k_weight", "patience", "max_it", "init_cross", "final_cross", "n_it"]
    #res = pd.DataFrame( columns=col_names )

    params = itertools.product(params['k_relax'], params['k_weight'], [params['patience']], [params['max_it']], gs)

    with Pool(2) as p:
        res = p.map(evaluateResults, params)
